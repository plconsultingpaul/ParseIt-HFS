    ...
    <div className="space-y-6">
      ...
      {processedEmails.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600">Processed emails will appear here once email monitoring is active.</p>
        </div>
      ) : (
        <div className="space-y-4">
          {processedEmails.map((email) => (
            <div key={email.id} className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                {email.pdfFilename && (
                  <div>
                    ...
                    <p className="text-gray-600">{email.pdfFilename}</p>
                  </div>
                )}
                {email.parseitId && (
                  <div>
                    <span className="font-medium text-gray-700">ParseIt ID:</span>
                    <p className="text-gray-600">{email.parseitId}</p>
                  </div>
                )}
                {email.processedAt && (
                  <div>
                    <span className="font-medium text-gray-700">Processed:</span>
                    <p className="text-gray-600">{new Date(email.processedAt).toLocaleString()}</p>
                  </div>
                )}
              </div>
              ...
              {email.errorMessage && (
                <div className="mt-4 bg-red-50 border border-red-200 rounded-lg p-3">
                  ...
                  <p className="text-red-700 text-sm">{email.errorMessage}</p>
                </div>
              )}
            </div>
          ))}
      ...
      {processedEmails.length > 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <h4 className="font-semibold text-blue-800 mb-2">Processing Statistics</h4>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div>
              <span className="font-medium text-blue-700">Total:</span>
              <p className="text-blue-600">{processedEmails.length}</p>
            </div>
            <div>
            ...
            <div>
              <span className="font-medium text-blue-700">Failed:</span>
              <p className="text-blue-600">
