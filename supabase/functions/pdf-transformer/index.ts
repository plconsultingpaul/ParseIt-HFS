      ...
      if (isPageGroupProcessing) {
        ...
        try {
          ...
          aiFields.forEach(field => {
            ...
            const currentValue = getValueByPath(responseData, field.fieldName)

            if (field.dataType === 'boolean' && currentValue !== undefined) {
              setValueByPath(responseData, field.fieldName, normalizeBooleanValue(currentValue))
            } else if ((field.dataType === 'string' || !field.dataType) && currentValue !== undefined) {
              // Convert string fields to uppercase
              if (typeof currentValue === 'string' && currentValue !== '') {
                setValueByPath(responseData, field.fieldName, currentValue.toUpperCase())
              }
            }
          })
