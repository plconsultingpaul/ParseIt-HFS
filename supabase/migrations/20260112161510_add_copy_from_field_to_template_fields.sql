/*
  # Add Copy From Field to Order Entry Template Fields
  
  1. Changes
    - Add `copy_from_field` column to `order_entry_template_fields` table
...
COMMENT ON COLUMN order_entry_template_fields.copy_from_field IS 'Field name to copy value from when that field is populated';
